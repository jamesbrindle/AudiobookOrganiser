...
namespace FfMpeg.Extensions
{
    ...
    public class AaxActivationClient
    {
        ...
        public static string GetActivationChecksum(string path)
        {
            ...
            {
                return FfProbe.GetChecksum(path);
            }
        }
        ...
        private static string ToHexString(byte[] source, int offset, int length)
        {
        ...
        public static string GetActivationBytes(string path, out string checksum)
        {
            ...
            if (string.IsNullOrEmpty(activationBytes))
            {
                ...
                try
                {
                    activationBytes = Task.Run(() => Instance.ResolveActivationBytesFromAudibleApplicationsRegistry().Result).Result;
                }
                catch
                {
            ...
            if (string.IsNullOrEmpty(activationBytes))
            {
                ...
                try
                {
                    activationBytes = Task.Run(() => Instance.ResolveActivationBytesFromAudibleCliProfile().Result).Result;
                }
                catch
                {
                    activationBytes = string.Empty;
            ...
            if (string.IsNullOrEmpty(activationBytes))
            {
                ...
                try
                {
                    activationBytes = Task.Run(() => Instance.ResolveActivationBytesUsingAudibleCliDirectly().Result).Result;
                }
                catch
                {
                    activationBytes = string.Empty;
               ...
               checksum != "0000000000000000000000000000000000000000")
            {
                ...
                try
                {
                    activationBytes = Task.Run(() => Instance.ResolveActivationBytesFromService(dChecksum).Result).Result;
                }
                catch
                {
                    activationBytes = string.Empty;
        ...
        /// <summary>
        /// Get Aax activation key from file checksum using an online service
        /// </summary>
        /// <param name="checksum">Aax file checksum</param>
        /// <returns>Activation Key</returns>
        public async Task<string> ResolveActivationBytesFromService(string checksum)
        {
            try
            {
                ...
                string[] parts = Regex.Split(response, "activationBytes\":\"");
        ...
        public async Task<string> ResolveActivationBytesUsingRCrack(string checksum)
        {
            try
            {
                ...
                string result = ProcessHelper.ExecuteProcessAndReadStdOut(
                    rcrackExePath,
                    out string _,
        ...
        public async Task<string> ResolveActivationBytesUsingAudibleCliDirectly()
        {
            try
            {
                string activationBytes = ProcessHelper.ExecuteProcessAndReadStdOut("audible.exe", out string _, "activation-bytes");
        ...
        /// <param name="path">Aax file path</param>
        /// <returns>Activation Key</returns>
        public static AudibleKeyAndIv GetAudibleKeyAndIv(string path, out string checksum)
        {
            checksum = string.Empty;
