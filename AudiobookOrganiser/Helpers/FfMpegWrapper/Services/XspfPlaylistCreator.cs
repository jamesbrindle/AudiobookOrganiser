...
{
    /// <summary>
    /// Create an Xsp playlist string contents for a file for a list of media files
    /// </summary>
    public class XspfPlaylistCreator : IPlaylistCreator
    {
        ...
        public string Create(IList<MetaData> metaData)
        {
            ...
            for (var i = 0; i < metaData.Count; i++)
            {
                playlist.TrackList[i] =
                    new Track
                    {
                        ...
                        Location =
                            $"file:///{metaData[i].FileInfo.FullName.Replace(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar)}",
            ...
            using (var xmlWriter =
                XmlWriter.Create(sw, new XmlWriterSettings { Indent = true, Encoding = Encoding.UTF8 }))
            {
    ...
    public class Playlist
    {
        ...
        [XmlElement("extension")] public PlaylistExtension Extension { get; set; }
    }
    ...
    public class Track
    {
        ...
        [XmlElement("duration")] public long Duration { get; set; }

        [XmlElement("extension")] public Extension Extension { get; set; }
    }
    ...
    public class Item
    {
        [XmlAttribute("tid")] public int TId { get; set; }
    }
}