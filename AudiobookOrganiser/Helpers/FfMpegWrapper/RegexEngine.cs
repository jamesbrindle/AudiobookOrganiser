...
namespace AudiobookOrganiser.Helpers.FfMpegWrapper
{
    ...
    internal static class RegexEngine
    {
        ...
        internal static bool TimeSpanLargeTryParse(string str, out TimeSpan result)
        {
            ...
            if (!double.TryParse(str.Substring(start), NumberStyles.Number, CultureInfo.InvariantCulture,
                out var seconds))
                return false;
        ...
        {
            return long.TryParse(match.Groups[1].Value, NumberStyles.Any, CultureInfo.InvariantCulture, out var result)
                ? result
                : (long?)null;
        }
        ...
        {
            return double.TryParse(match.Groups[1].Value, NumberStyles.Any, CultureInfo.InvariantCulture,
                out var result)
                ? result
                : (double?)null;
        }
        ...
        {
            return int.TryParse(match.Groups[1].Value, NumberStyles.Any, CultureInfo.InvariantCulture, out var result)
                ? result
                : (int?)null;
        }
        ...
        internal static void TestAudio(string data, FfMpegParameters engine)
        {
            ...
            if (engine.InputFile.MetaData.AudioData == null)
                engine.InputFile.MetaData.AudioData = new MetaData.Audio
                {
                    ...
                    BitRateKbs = !string.IsNullOrWhiteSpace(matchAudioBitRate[1].ToString())
